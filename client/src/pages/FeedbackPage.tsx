...
function ReviewForm({ review, orders, open, onOpenChange, defaultOrderId }: { review?: Review | null; orders: OrderWithServices[]; open: boolean; onOpenChange: (open: boolean) => void; defaultOrderId?: number | null }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto border-slate-800 bg-slate-900 text-white sm:max-w-xl"><DialogHeader><DialogTitle>{review ? "Update Review" : "New Review"}</DialogTitle></DialogHeader>
    <div className="space-y-4">
      ...
      {review && user?.role === "admin" && <div className="space-y-2"><Label>Review progress</Label><Select value={reviewProgress} onValueChange={value => setReviewProgress(value as Review["reviewProgress"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="requested">Requested</SelectItem><SelectItem value="received">Received</SelectItem><SelectItem value="public_review_received">Public Review Received</SelectItem><SelectItem value="closed">Closed</SelectItem></SelectContent></Select><p className="text-xs text-slate-500">Progress advances automatically when feedback is recorded. Admins can correct it when needed.</p></div>}
...
function ReviewProgress({ review }: { review: Review }) {
  const steps: Array<{ value: Review["reviewProgress"]; label: string }> = [{ value: "requested", label: "Requested" }, { value: "received", label: "Received" }, { value: "public_review_received", label: "Public Review" }, { value: "closed", label: "Closed" }];
  ...
  return <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3">
    ...
    <p className="mt-2 text-xs text-slate-500">Requested means follow-up is pending; Received means private feedback arrived; Public Review means a public link, Facebook review, or video was received; Closed completes follow-up.</p>
  </div>;
}
...
function SuggestionDetails({ suggestion, open, onOpenChange, onOpenOrder, onUpdated }: { suggestion: Suggestion | null; open: boolean; onOpenChange: (open: boolean) => void; onOpenOrder: (id: number) => void; onUpdated: (suggestion: Suggestion) => void }) {
  ...
  const update = useMutation({
    mutationFn: async (status: Suggestion["status"]) => (await apiRequest("PATCH", `/api/feedback/suggestions/${suggestion?.id}`, { status })).json(),
    onSuccess: (updated: Suggestion) => { onUpdated(updated); queryClient.invalidateQueries({ queryKey: ["/api/feedback"] }); queryClient.invalidateQueries({ queryKey: ["/api/feedback/stats"] }); toast({ title: "Suggestion status updated" }); },
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="w-full overflow-y-auto border-slate-800 bg-slate-950 text-white sm:max-w-xl"><SheetHeader><SheetTitle>{suggestion?.suggestionNumber || "Suggestion Details"}</SheetTitle></SheetHeader>{suggestion && <div className="mt-6 space-y-5">
    ...
    {user?.role === "admin" && <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4"><Label>Suggestion status</Label><Select value={suggestion.status} onValueChange={value => update.mutate(value as Suggestion["status"])} disabled={update.isPending}><SelectTrigger className="mt-2"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(suggestionStatusLabels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select><p className="mt-2 text-xs text-slate-500">New: untriaged · Under Review: being assessed · Accepted: approved for work · Implemented: completed · Rejected: not proceeding.</p></section>}
