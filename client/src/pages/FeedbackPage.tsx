...
function MetricCard({ title, value, icon: Icon, color = "blue", testId }: { title: string; value: string | number; icon: typeof Star; color?: string; testId?: string }) {
  ...
  return (
    <div className="glass-panel relative flex min-h-[108px] flex-col justify-between overflow-hidden rounded-2xl border border-slate-800/80 p-4 transition-colors hover:border-slate-700" data-testid={testId}>
      <div className="flex items-center gap-3">
        <div className={`rounded-xl p-2.5 ${colors[color] || colors.blue}`}><Icon className="h-5 w-5" /></div>
        <p className="truncate text-xs font-medium uppercase tracking-wide text-slate-400">{title}</p>
      </div>
      <p className="pl-1 text-2xl font-bold leading-none text-white">{value}</p>
      <div className={`pointer-events-none absolute -right-8 -top-8 h-20 w-20 rounded-full blur-2xl ${colors[color] || colors.blue}`} aria-hidden="true" />
    </div>
  );
}
...
export default function FeedbackPage() {
  ...
  return <div className="space-y-4 p-4 md:space-y-8 md:p-8">
    ...
    {deepLinkMessage && <div className="flex items-center justify-between gap-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-100"><span>{deepLinkMessage}</span><Button variant="ghost" size="sm" onClick={() => { setDeepLinkMessage(null); setLocation(`/feedback?tab=${tab}`); }}>Clear link</Button></div>}
     ...
     <Tabs value={tab} onValueChange={value => setTab(value as "reviews" | "suggestions")}><TabsList className="w-full justify-start gap-1 rounded-xl border border-slate-800 bg-slate-900/70 p-1 sm:w-fit"><TabsTrigger value="reviews" className="gap-2 rounded-lg px-5 py-2 data-[state=active]:bg-primary data-[state=active]:text-white"><Star className="h-4 w-4" />Reviews</TabsTrigger><TabsTrigger value="suggestions" className="gap-2 rounded-lg px-5 py-2 data-[state=active]:bg-primary data-[state=active]:text-white"><ThumbsUp className="h-4 w-4" />Suggestions</TabsTrigger></TabsList>
...
function FeedbackFilters({ designers, showDesignerFilter, value, onChange, status, onStatusChange }: { designers: User[]; showDesignerFilter: boolean; value: string; onChange: (value: string) => void; status?: string; onStatusChange?: (value: string) => void }) {
 ...
 function FeedbackResultsHeader({ title, count, month, year, setYear, monthValue, setMonth }: { title: string; count: number; month: string; year: string; setYear: (value: string) => void; monthValue: string; setMonth: (value: string) => void }) {
  const now = new Date();
  return <div className="flex flex-col items-start justify-between gap-5 border-b border-slate-800/80 px-5 py-5 sm:flex-row sm:items-center sm:px-6">
    <div><h3 className="text-lg font-bold text-white">{title}</h3><p className="mt-1 text-sm text-slate-500">{count} {title.toLowerCase().replace(/s$/, "")}{count === 1 ? "" : "s"} for {month}</p></div>
