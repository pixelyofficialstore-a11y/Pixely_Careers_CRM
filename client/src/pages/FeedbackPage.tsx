...
export default function FeedbackPage() {
  ...
  const reportMonth = format(new Date(Number(year), Number(month) - 1, 1), "MMMM yyyy");
   return <div className="crm-page space-y-5">
     <Tabs value={tab} onValueChange={value => setTab(value as "reviews" | "suggestions")} className="space-y-5">
       ...
       <div className="flex w-full flex-col gap-3 xl:w-auto xl:min-w-[680px]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative min-w-0 flex-1 sm:min-w-[280px]">
            ...
            <Input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search feedback, order, client, or designer..." className="crm-control h-10 pl-10 text-white" />
          </div>
         ...
         <div className="flex flex-wrap items-center justify-between gap-2">
           <TabsList className="w-full justify-start border border-slate-800 bg-slate-900/70 p-1 sm:w-fit">
             <TabsTrigger value="reviews" className="gap-2 px-5"><Star className="h-4 w-4" />Reviews</TabsTrigger>
             <TabsTrigger value="suggestions" className="gap-2 px-5"><ThumbsUp className="h-4 w-4" />Suggestions</TabsTrigger>
           </TabsList>
    ...
    {deepLinkMessage && <div className="flex items-center justify-between gap-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-100"><span>{deepLinkMessage}</span><Button variant="ghost" size="sm" onClick={() => { setDeepLinkMessage(null); setLocation(`/feedback?tab=${tab}`); }}>Clear link</Button></div>}
      <div className={`grid grid-cols-1 gap-3 sm:grid-cols-2 ${tab === "reviews" ? "xl:grid-cols-5" : "xl:grid-cols-4"}`}>
