...
const complaintHistoryLabel = (entry: ComplaintHistoryEntry) => {
  ...
  if (entry.action === "complaint_resolved") return `Complaint marked ${to || "resolved"}.`;
  if (entry.action === "complaint_note") return `Admin note ${entry.previousValue ? "updated" : "added"}.`;
...
function ComplaintDrawer({ id, open, onOpenChange }: { id: number | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [notes, setNotes] = useState(""); const [resolution, setResolution] = useState(""); const [resolutionEvidence, setResolutionEvidence] = useState(""); const [dismissalReason, setDismissalReason] = useState(""); const [refundConfirmed, setRefundConfirmed] = useState(false); const [pending, setPending] = useState<string | null>(null);
  ...
  const terminal = complaint && ["dismissed", "resolved", "refunded"].includes(complaint.status);
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="w-full sm:max-w-xl overflow-y-auto bg-slate-950 border-slate-800 text-white">
    ...
    {isLoading ? <div className="py-24 text-center"><Loader2 className="mx-auto animate-spin text-blue-400" /></div> : isError || !complaint ? <div className="py-20 text-center text-slate-400"><AlertTriangle className="mx-auto mb-3 text-rose-400" />Could not load this complaint.</div> : <div className="space-y-5 mt-6">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500 mb-3">Evidence</p>{complaint.screenshotUrl ? <a href={complaint.screenshotUrl} target="_blank" rel="noreferrer" className="block"><img src={complaint.screenshotUrl} alt="Complaint evidence" className="max-h-56 w-full rounded-lg bg-slate-950 object-contain" onError={e => { e.currentTarget.style.display = "none"; }} /></a> : <p className="text-sm text-slate-500">No complaint evidence attached.</p>}</section>
      {(complaint.resolution || complaint.resolutionOutcome) && <section className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4"><p className="text-xs uppercase tracking-wider text-emerald-300">Resolution</p><p className="mt-2 text-sm text-slate-300">{complaint.resolution || "—"}</p>{complaint.resolutionScreenshotUrl && <a href={complaint.resolutionScreenshotUrl} target="_blank" rel="noreferrer"><img src={complaint.resolutionScreenshotUrl} alt="Resolution evidence" className="mt-3 max-h-56 w-full rounded-lg object-contain" /></a>}</section>}
      {complaint.status === "dismissed" && <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Reason for Dismissal</p><p className="mt-2 whitespace-pre-wrap text-sm text-slate-300">{complaint.dismissalReason || "No reason recorded."}</p></section>}
      {isAdmin && <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 space-y-4"><div><p className="text-xs uppercase tracking-wider text-slate-500">Decision controls</p><p className="text-xs text-slate-500 mt-1">Every decision is recorded and cannot be reversed.</p></div>
        {complaint.status === "new" && <div className="grid grid-cols-2 gap-2"><Button onClick={() => setPending("confirmed")} className="bg-amber-600 hover:bg-amber-500"><CheckCircle2 className="w-4 h-4 mr-2" />Confirm Complaint</Button><Button variant="outline" onClick={() => setPending("dismissed")}><XCircle className="w-4 h-4 mr-2" />Dismiss Complaint</Button></div>}
        {complaint.status === "confirmed" && <><div className="space-y-2"><Label htmlFor="resolution">Resolution Details <span className="text-rose-400">*</span></Label><Textarea id="resolution" value={resolution} onChange={e => setResolution(e.target.value)} placeholder="What action was taken to resolve the complaint?" rows={3} className="bg-slate-950 border-slate-700" /></div><Label className="block cursor-pointer text-sm">Upload resolution evidence <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={e => { const file = e.target.files?.[0]; if (file) void uploadResolutionEvidence(file); }} /></Label>{resolutionEvidence && <img src={resolutionEvidence} alt="Selected resolution evidence" className="max-h-40 w-full rounded-lg object-contain" />}<label className="flex gap-2 text-sm text-slate-300"><input type="checkbox" checked={refundConfirmed} onChange={e => setRefundConfirmed(e.target.checked)} />I confirm that the client refund has been completed.</label><div className="grid grid-cols-2 gap-2"><Button disabled={!resolution.trim()} onClick={() => setPending("resolved")} className="bg-emerald-600 hover:bg-emerald-500">Mark Resolved</Button><Button variant="outline" disabled={!resolution.trim() || !refundConfirmed} onClick={() => setPending("refunded")}>Mark Refunded & Cancel Order</Button></div></>}
        {terminal && <p className="rounded-lg bg-slate-950 p-3 text-sm text-slate-500">This complaint is closed and no further action is required.</p>}
        <div className="border-t border-slate-800 pt-4 space-y-2"><Label htmlFor="admin-notes">Admin Notes</Label><p className="text-xs text-slate-500">Private notes for management. Not visible to Designers or Support.</p><Textarea id="admin-notes" value={notes} onChange={e => setNotes(e.target.value)} placeholder={complaint.adminNotes || "Visible only to admins"} rows={3} className="bg-slate-950 border-slate-700" /><Button variant="outline" disabled={!notes.trim() || update.isPending} onClick={() => update.mutate({ adminNotes: notes.trim() })}>Save note</Button></div>
      </section>}
      {isAdmin && <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">History</p><div className="mt-4 space-y-4">{history.length ? history.map(entry => <div key={entry.id} className="relative border-l border-slate-700 pl-4"><span className="absolute -left-1.5 top-1 h-3 w-3 rounded-full border-2 border-slate-900 bg-blue-500" /><p className="text-sm">{complaintHistoryLabel(entry)} {entry.actor?.name ? `by ${entry.actor.name}` : ""}</p><p className="text-xs text-slate-500 mt-1">{entry.createdAt ? format(new Date(entry.createdAt), "MMM dd, yyyy · h:mm a") : "—"}</p></div>) : <p className="text-sm text-slate-500">No history entries yet.</p>}</div></section>}
    </div>}
