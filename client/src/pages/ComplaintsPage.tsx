    ...
    {isLoading ? <div className="py-24 text-center"><Loader2 className="mx-auto animate-spin text-blue-400" /></div> : isError || !complaint ? <div className="py-20 text-center text-slate-400"><AlertTriangle className="mx-auto mb-3 text-rose-400" />Could not load this complaint.</div> : <div className="mt-6 space-y-5">
      ...
      {complaint.order && <section className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5">
        ...
        {(complaint.order.totalPrice != null || complaint.order.advanceAmount != null || complaint.order.remainingAmount != null) && <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-800 pt-4 text-sm">
          ...
          {complaint.order.advanceAmount != null && <div><dt className="text-xs text-slate-500">Advance</dt><dd className={`mt-1 text-white ${complaint.order.status === "canceled" ? "line-through decoration-rose-400" : ""}`}>{money(complaint.order.advanceAmount)}</dd>{complaint.order.status === "canceled" && <span className="text-xs text-rose-300">Refunded</span>}</div>}
          {complaint.order.remainingAmount != null && <div><dt className="text-xs text-slate-500">Remaining</dt><dd className={`mt-1 text-white ${complaint.order.status === "canceled" ? "line-through decoration-slate-500" : ""}`}>{money(complaint.order.remainingAmount)}</dd>{complaint.order.status === "canceled" && <span className="text-xs text-slate-500">Canceled</span>}</div>}
        </dl>}
      </section>}
      ...
      {complaint.status === "confirmed" && <section className="space-y-4 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-300">Resolution</p><p className="mt-2 text-sm leading-6 text-slate-400">This complaint has been confirmed. Record how the issue was handled before closing the case.</p></div>{isAdmin && <><div className="space-y-2"><Label htmlFor="resolution">Resolution Details <span className="text-rose-400">*</span></Label><Textarea id="resolution" value={resolution} onChange={event => setResolution(event.target.value)} placeholder="What action was taken to resolve the complaint?" rows={4} className="border-slate-700 bg-slate-950" /></div><Label className="block cursor-pointer text-sm text-slate-300">Upload Resolution Evidence<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={event => { const file = event.target.files?.[0]; if (file) void uploadResolutionEvidence(file); }} /></Label>{resolutionEvidence && <img src={resolutionEvidence} alt="Selected resolution evidence" className="max-h-40 w-full rounded-lg object-contain" />}<label className="flex gap-2 text-sm text-slate-300"><input type="checkbox" checked={refundConfirmed} onChange={event => setRefundConfirmed(event.target.checked)} />I confirm that the client refund has been completed.</label><div className="grid grid-cols-2 gap-2"><Button disabled={!resolution.trim()} onClick={() => setPending("resolved")} className="bg-emerald-600 hover:bg-emerald-500">Mark Resolved</Button><Button variant="outline" disabled={!resolution.trim() || !refundConfirmed} onClick={() => setPending("refunded")}>Mark Refunded &amp; Cancel Order</Button></div></>}</section>}
